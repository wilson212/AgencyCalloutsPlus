...
namespace AgencyCalloutsPlus.Mod.NativeUI
{
    ...
    internal class ADACMainMenu
    {
        ...
        private void RequestCallMenuButton_Activated(UIMenu sender, UIMenuItem selectedItem)
        {
            ...
            {
                Game.DisplayNotification("~r~You are currently not available for calls!");
            }
            else
            {
                if (!dispatched)
                {
                    Game.DisplayNotification("There are no calls currently available. ~g~Dispatch will send you the next call that comes in");
                }
            }
        }
